...
from data import AudioVisualDataLoader, AudioVisualDataset
...
parser.add_argument('--aud_LFR_m', default=4, type=int,
                    help='Low Frame Rate: number of frames to stack (for audio)')
parser.add_argument('--aud_LFR_n', default=3, type=int,
                    help='Low Frame Rate: number of frames to skip (for audio)')
parser.add_argument('--vid_LFR_m', default=1, type=int,
                    help='Low Frame Rate: number of frames to stack (for video)')
parser.add_argument('--vid_LFR_n', default=1, type=int,
                    help='Low Frame Rate: number of frames to skip (for video)')
...
parser.add_argument('--init_audio_weight', default=1.0, type=float,
                    help='Initial audio weight')
# Loss
...
def main(args):
    # Construct Solver
    # data
    tr_dataset = AudioVisualDataset(args.train_json, args.batch_size,
                              args.maxlen_in, args.maxlen_out,
                              batch_frames=args.batch_frames)
    cv_dataset = AudioVisualDataset(args.valid_json, args.batch_size,
                              args.maxlen_in, args.maxlen_out,
                              batch_frames=args.batch_frames)
    tr_loader = AudioVisualDataLoader(tr_dataset, batch_size=1,
                                num_workers=args.num_workers,
                                shuffle=args.shuffle,
                                aud_LFR_m=args.aud_LFR_m, aud_LFR_n=args.aud_LFR_n,
                                vid_LFR_m=args.vid_LFR_m, vid_LFR_n=args.vid_LFR_n)
    ...
    # model
    audio_encoder = Encoder(args.d_input * args.aud_LFR_m, args.n_layers_enc, args.n_head,
                      args.d_k, args.d_v, args.d_model, args.d_inner,
                      dropout=args.dropout, pe_maxlen=args.pe_maxlen)
    video_encoder = VideoEncoder(args.d_input * args.vid_LFR_m, args.n_layers_enc, args.n_head,
                      args.d_k, args.d_v, args.d_model, args.d_inner,
                      dropout=args.dropout, pe_maxlen=args.pe_maxlen)
    decoder = Decoder(sos_id, eos_id, vocab_size,
                      ...
                      audio_weight=args.init_audio_weight,
                      pe_maxlen=args.pe_maxlen)
    model = Transformer(audio_encoder, video_encoder, decoder)
    print(model)
    model.cuda()
    # optimizer
    optimizier = TransformerOptimizer(
        #torch.optim.Adam(model.parameters(), betas=(0.9, 0.98), eps=1e-09),
        RAdam(model.parameters()),
        args.k,
        args.d_model,
        args.warmup_steps)
